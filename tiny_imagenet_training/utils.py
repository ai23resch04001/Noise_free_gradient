...
def get_dataset(program_args,train=True,gradient_generation=False,gradient_calculation=False,shuffle=True):
    ...
    elif program_args["dataset"]=="tiny":
        num_classes = 200
    ...
    elif program_args["dataset"]=="tinyn":
        num_classes = 200
        mean = [0.4802, 0.4481, 0.3975]
        std = [0.2302, 0.2265, 0.2262]
        data_root = "data/tiny-imagenet-200"
        print("IN noisy dataset")
        if gradient_generation:  # No test data is required
            ...
            data_root = os.path.join(data_root,"train")
            train_dataset  = torchvision.datasets.ImageFolder(root=data_root, transform=train_transform)
            train_dataset.targets = np.load("tiny_imagenet_training/datasets/tiny_target.npy")
            print(train_dataset)
            train_loader = DataLoader(
                        train_dataset,
                        batch_size=program_args["batch_size"], # may need to reduce this depending on your GPU 
...
def get_model(program_args):
    ...
    elif program_args["dataset"]=="tiny" or program_args["dataset"]=="tinyn":
        num_classes = 200
        im_size=64
        patch_size=8
    else:
    ...
    elif program_args["model"]=="resnet18" and program_args["dataset"]=="tiny":
    ...
    elif program_args["model"]=="resnet50" and (program_args["dataset"]=="tiny" or program_args["dataset"]=="tinyn"):
        channel = 3 
        num_classes = 200
        record_embedding = False 
        no_grad = False
        model = resnet.ResNet_32x32(resnet.Bottleneck, [3, 4, 6, 3], channel=channel, num_classes=num_classes,
    ...
    elif program_args["model"]=="resnet50" and "tiny" not in program_args["dataset"]:
